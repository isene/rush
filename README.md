...
**Editing:**
- Full line editing: cursor movement, insert, delete, word-delete
- Syntax highlighting: commands (green), unknown (red), switches (yellow), paths, nicks, bookmarks
...
- Customizable prompt colors (user, host, cwd, git branch, prompt char)
...
- Command timing with slow-command alerts
...
| Key | Action |
|-----|--------|
| Tab | Complete (interactive cycling) |
...
```json
{
  ...
  "dir_colors": [["MyProject", 172], ["/G", 172]],
...
|---|---|---|
| Startup | ~300ms | ~26ms |
| Binary | Ruby runtime | 2.4MB standalone |
| Config | Ruby eval (.rshrc) | JSON (.rushrc.json) |
| Plugins | Ruby files | Not yet |
| Features | 76 | 76 |
